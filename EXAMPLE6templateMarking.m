...
if(strcmp(md5(strcat(pwd,'\BotSim.m')), '8D3E7E7E8AECDF2B49A8B29648DAE1B9') ==0)
        ...
        for j=1:size(noiseLevel,2)
            ...
            for k = 1:numberOfrepeats
                ...
                if k ==1 %runs from preset start and target positions first time, after that choose random positions
                    ...
                    botSim.randomPose(10,adminKey); %puts the robot in a random position at least 10cm away from a wall
                ...
                %% Subdivide path and Calculate path length and if bot collides
                subDivPath = subDividePath(path,5);
                if size(subDivPath,1) >0
                    for m = 1:size(subDivPath,1)-1
                        if ~botSim.pointInsideMap(subDivPath(m,:))
                            collided = 1;
                        end
                        pathLength = pathLength + distance(subDivPath(m,:),subDivPath(m+1,:));
                    end
                    if ~botSim.pointInsideMap(subDivPath(size(subDivPath,1),:))
                        collided = 1;
                    end
                end
